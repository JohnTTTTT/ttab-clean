...
class AffectNetDataset(PyTorchDataset):
    def __init__(
        ...
        # pull the stats & size from your ttab.config
        stats = dataset_defaults["affectnet"]["statistics"]
        img_h, img_w, _ = dataset_defaults["affectnet"]["img_shape"]

        # normalize must be a dict of mean/std, just like in CIFAR/ImageNet
        normalize = {"mean": stats["mean"], "std": stats["std"]}

        self.transform = get_transform(
            "affectnet",            # reuse the ImageNet branch for a 224×224 crop
            input_size=img_h,      # 224
            augment=False,
        )
        self.target_transform = None
        ...
        super().__init__(
            dataset,
            device=device,
            prepare_batch=AffectNetDataset.prepare_batch,
            num_classes=num_classes,
        )
